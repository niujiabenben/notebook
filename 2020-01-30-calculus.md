...
注意到全微分的含义是在某点的邻域内用切平面近似曲面, 且有点 $(x,y,z)$ 和 点 $(x+dx,y+dy,z+dy)$ 都在切平面上, 所以向量 $(dx,dy,dz)$ 平行于切平面. 又由于 $dx$ 和 $dy$ 在邻域内可以任意取值, 而上面的式子总是成立, 所以 $(\frac{\partial z}{\partial x},\frac{\partial z}{\partial y},-1)$ 为平面的法线.
